...
export default function AssignTemplatesToSchoolDialog({ schoolId, children }: { schoolId: number; children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const fetchTemplatesFromAPI = async () => {
      try {
        const response = await fetch(`/api/templates?is_custom=false`);
  ...
  const assign = async () => {
    ...
    const res = await assignTemplateToSchoolsAction({ school_id: schoolId, template_ids: templateIds });
    if (!res.success) {
      ...
      <DialogContent className="sm:max-w-6xl w-[95vw] max-h-[90vh] flex flex-col bg-white">
        <DialogHeader className="pb-6 border-b border-gray-200">
          <DialogTitle className="text-2xl font-semibold text-gray-900 flex items-center gap-3">
            ...
            Assign Custom Templates to School
          </DialogTitle>
          <p className="text-gray-600 mt-2">Select the custom templates you want to assign to this school</p>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto border border-gray-200 rounded-lg ">
          <section className="w-full flex items-center bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200 sticky top-0 z-10">
            ...
            <div className="w-[25%] px-4 py-3  text-left font-semibold text-gray-700 text-sm uppercase tracking-wide">Template Name</div>
            <div className="w-[10%] px-4 py-3 text-left font-semibold text-gray-700 text-sm uppercase tracking-wide">Type</div>
            <div className="w-[40%] px-4 py-3 text-left font-semibold text-gray-700 text-sm uppercase tracking-wide">Subject</div>
            <div className="w-[15%] text-left font-semibold text-gray-700 text-sm uppercase tracking-wide ">Category</div>
          ...
          <section className="w-full overflow-y-auto">
            {filtered.map((t, index) => {
              ...
              return (
                <div
                  key={t.id}
                  className={`w-full flex items-center border-b border-gray-100 transition-all duration-200 hover:bg-gray-50 ${isSelected ? 'bg-blue-50 border-blue-200' : index % 2 === 0 ? 'bg-white' : 'bg-gray-50'
                    }`}
                >
                  ...
                  <div className="w-[40%] px-4 py-3">
                    <div className="text-gray-700 truncate" title={t.subject}>{t.subject}</div>
                  </div>

                  <div className="w-[15%]">
                    <span className={` gap-1 px-2 py-1 rounded-full text-xs font-medium  ${t.is_custom
            ...
            {filtered.length === 0 && (
              <div className="px-6 py-12 text-center">
                <div className="text-gray-500 text-lg font-medium">No custom templates found</div>
                <p className="text-gray-400 text-sm mt-2">Try adjusting your search criteria or check if custom templates exist</p>
              </div>
            )}
          </section>
        </div>
        ...
        <div className="flex justify-end gap-4 pt-6 border-t border-gray-200 mt-6">
          ...
          >
            Assign {selected.size > 0 ? `(${selected.size})` : ''} Template{selected.size !== 1 ? 's' : ''}
          </CommonButton>
        </div>
      </DialogContent>
    </Dialog>
