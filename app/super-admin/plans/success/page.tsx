...
import { fetchSubscriptionBySessionId, fetchSubscriptionBySessionIdforSuperAdmin, type SubscriptionNormalized } from '@/lib/admin/subscription'
...
export default async function SuccessPage({searchParams}: {searchParams: Promise<{ [key: string]: string | string[] | undefined }>}) {
  ...
  if (session_id) {
    const res = await fetchSubscriptionBySessionIdforSuperAdmin(session_id);
    if (res.success) sub = res.data;
  }
  const redirectUrl = schoolId ? `/super-admin/school-users?schoolId=${schoolId}` : '/super-admin/schools';

  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 flex items-center justify-center h-full">
    
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/20 overflow-hidden h-full">
          {/* Header Section */}
          ...
          <div className="p-8">
                ...
                <div className="space-y-4">
                  ...
                  {schoolId && (
                    <div className="flex justify-between items-center py-3 px-4 bg-white/60 rounded-xl border border-white/50">
                      ...
                      <span className="font-bold text-gray-900">{schoolId}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* Action Buttons */}
            <div className="space-y-4">
              <Link href={redirectUrl} className="w-full flex items-center justify-center gap-3 bg-gradient-to-r from-[#8D8EF5] to-[#6366F1] text-white hover:from-[#8D8EF5]/90 hover:to-[#6366F1]/90 h-12 rounded-xl font-semibold text-lg shadow-lg hover:shadow-xl transition-all duration-300">
